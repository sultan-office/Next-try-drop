...
const DeleteModal = ({
  ...
  const handleDelUiIn = () => {
    ...
    setTimeout(() => {
      domMsg.current.style.display = "block";
      btnBlock.current.style.display = "block";
      domMsg.current.style.opacity = "1";
      btnBlock.current.style.opacity = "1";
      setShowModal(false);
      setDelSuccess(false);
    }, 2000);
  };
      ...
      <MagnifyPopup show={showModal} handleHide={setShowModal}>
        <div id="delete-popup" className="">
          ...
          <p
            ...
            style={{ display: `${delSuccess ? "block" : "none"}` }}
          >
            {successMessage}
          </p>
        </div>
